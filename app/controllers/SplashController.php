 	...
 	class SplashController extends BaseBrowseController {
 		...
 		function Index() {
			...
			$t_featured->load(array('set_code' => $this->request->config->get('featured_set_name')));
 			...
 			if(!is_array($va_featured_ids) || (sizeof($va_featured_ids) == 0)){
				# put a random object in the features variable
 				$va_featured_ids = array_keys($t_object->getRandomItems(20, array('checkAccess' => $va_access_values, 'hasRepresentations' => 1)));
			}
			...
			$va_recently_added_items = $t_object->getRecentlyAddedItems(12, array('checkAccess' => $va_access_values, 'hasRepresentations' => 1));
 			...
 			$va_media = $t_object->getPrimaryMediaForIDs(array_keys($va_recently_added_items), array('small', 'splashthumb', 'preview', 'widepreview', 'medium'), array("checkAccess" => $va_access_values));
			...
			if(is_array($va_recently_added_objects) && (sizeof($va_recently_added_objects) > 0)){
				...
				$this->view->setVar('recently_added_splashthumb', $va_media[$va_object_info['object_id']]["tags"]["splashthumb"]);
			...
			$va_recently_viewed_objects = $t_object->getRecentlyViewedItems(12, array('checkAccess' => $va_access_values, 'hasRepresentations' => 1));
 			...
 			$va_media = $t_object->getPrimaryMediaForIDs($va_recently_viewed_objects, array('small', 'splashthumb', 'preview', 'widepreview', 'medium'), array("checkAccess" => $va_access_values));
			...
			if(is_array($va_recently_viewed_objects) && (sizeof($va_recently_viewed_objects) > 0)){
				...
				$this->view->setVar('recently_viewed_splashthumb', $va_media[$va_object_info['object_id']]["tags"]["splashthumb"]);
			...
			$va_labels = $t_object->getPreferredDisplayLabelsForIDs(array_keys($va_random_items));
			$va_media = $t_object->getPrimaryMediaForIDs(array_keys($va_random_items), array('small', 'splashthumb', 'preview','medium', 'widepreview'), array("checkAccess" => $va_access_values));
			...
			if(is_array($va_random_items) && (sizeof($va_random_items) > 0)){
				...
				$this->view->setVar('random_object_splashthumb', $va_media[$va_object_info['object_id']]["tags"]["splashthumb"]);
