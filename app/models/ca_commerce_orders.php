...
class ca_commerce_orders extends BaseModel {
	...
	public function insert($pa_options=null) {
		...
		if ($vn_rc = parent::insert($pa_options)) {
			$this->sendEmailNotification(null, null, null);
		}
		return $vn_rc;
	}
	# ----------------------------------------
	public function update($pa_options=null) {
		...
		if($vn_rc = parent::update($pa_options)) {
			$this->sendEmailNotification($vn_old_status, $vn_old_ship_date, $vn_old_shipped_on_date);
		}
		return $vn_rc;
	}
	# ----------------------------------------
	 ...
	 *
	 */
	public function sendEmailNotification($pn_old_status, $pn_old_ship_date, $pn_old_shipped_on_date) {
		global $g_request;
		...
		if ($vb_status_has_changed) {	// has status changed?
			switch($vs_status) {
				case 'SUBMITTED':
					...
					caSendMessageUsingView($g_request, $vs_to_email, $vs_sender_email, "[{$vs_app_name}] {$vs_subject}", "commerce_order_status_submitted.tpl", array('subject' => $vs_subject, 'from_user_id' => $g_request->getUserID(), 'sender_name' => $vs_sender_name, 'sender_email' => $vs_sender_email, 'sent_on' => time(), 'login_url' => $vs_login_url, 't_order' => $this));
					break;
				case 'AWAITING_PAYMENT':
					$vs_subject = _t('Your order (%2) posted on %1 requires payment', $vs_order_date, $this->getPrimaryKey());
					caSendMessageUsingView($g_request, $vs_to_email, $vs_sender_email, "[{$vs_app_name}] {$vs_subject}", "commerce_order_status_awaiting_payment.tpl", array('subject' => $vs_subject, 'from_user_id' => $g_request->getUserID(), 'sender_name' => $vs_sender_name, 'sender_email' => $vs_sender_email, 'sent_on' => time(), 'login_url' => $vs_login_url, 't_order' => $this));
					break;
				case 'PROCESSED':
					$vs_subject = _t('Payment for order (%2) posted on %1 has been processed', $vs_order_date, $this->getPrimaryKey());
					caSendMessageUsingView($g_request, $vs_to_email, $vs_sender_email, "[{$vs_app_name}] {$vs_subject}", "commerce_order_status_processed.tpl", array('subject' => $vs_subject, 'from_user_id' => $g_request->getUserID(), 'sender_name' => $vs_sender_name, 'sender_email' => $vs_sender_email, 'sent_on' => time(), 'login_url' => $vs_login_url, 't_order' => $this));
					break;
				case 'COMPLETED':
					$vs_subject = _t('Your order (%2) posted on %1 is complete', $vs_order_date, $this->getPrimaryKey());
					caSendMessageUsingView($g_request, $vs_to_email, $vs_sender_email, "[{$vs_app_name}] {$vs_subject}", "commerce_order_status_completed.tpl", array('subject' => $vs_subject, 'from_user_id' => $g_request->getUserID(), 'sender_name' => $vs_sender_name, 'sender_email' => $vs_sender_email, 'sent_on' => time(), 'login_url' => $vs_login_url, 't_order' => $this));
					break;
				case 'REOPENED':
					$vs_subject = _t('Order (%2) posted on %1 has been reopened', $vs_order_date, $this->getPrimaryKey());
					caSendMessageUsingView($g_request, $vs_to_email, $vs_sender_email, "[{$vs_app_name}] {$vs_subject}", "commerce_order_status_reopened.tpl", array('subject' => $vs_subject, 'from_user_id' => $g_request->getUserID(), 'sender_name' => $vs_sender_name, 'sender_email' => $vs_sender_email, 'sent_on' => time(), 'login_url' => $vs_login_url, 't_order' => $this));
					break;
			}
		} else {
			...
			if ($vb_shipping_has_changed) {
				...
				if (($vn_shipped_on_date > 0) && ($vn_shipped_on_date != $pn_old_shipped_on_date)) {
					...
					$vs_subject = _t('Order (%2) posted on %1 has shipped', $vs_order_date, $this->getPrimaryKey());
				...
				if (($vn_ship_date > 0) && ($vn_ship_date != $pn_old_ship_date)) {
					...
					$vs_subject = _t('Order (%2) posted on %1 has been scheduled for shipping', $vs_order_date, $this->getPrimaryKey());
	...
	public function addItem($pn_object_id, $pa_values, $pa_options=null) {
	 	...
	 	$t_item->addRepresentations(array($vn_rep_id));
