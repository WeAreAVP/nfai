...
<?php
	if ($vn_ship_date = $t_order->get('shipped_on_date', array('GET_DIRECT_DATE' => true))) {
		// Already shipped
?>
		The order was shipped via <?php print $vs_shipping_method; ?> on <?php print date('F j, Y', $vn_ship_date); ?>.
<?php	
	} else {
		if ($vn_ship_date = $t_order->get('shipping_date', array('GET_DIRECT_DATE' => true))) {
			// Not yet shipped but expected shipping date known
?>
			The order will be shipped via <?php print $vs_shipping_method; ?> on <?php print date('F j, Y', $vn_ship_date); ?>.
<?php		
		} else {
			// Shipping data not known yet.
?>
			The order will be shipped via <?php print $vs_shipping_method; ?>. You will receive an email when the expected ship date is determined.
<?php
		}
	}
?>
