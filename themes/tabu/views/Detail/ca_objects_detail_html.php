...
<?php					$va_related_objects_by_type = array();
					foreach($va_related_objects as $va_info){
						$va_related_objects_by_type[$va_info["item_type_id"]][] = $va_info["object_id"];
					}
					
					// add current object
					$va_related_objects_by_type[$t_object->get("type_id")][] = $t_object->get("object_id");
					
					$va_all_results = $va_related_objects_by_type[$t_object->get("type_id")];
					if ((sizeof($va_all_results) > 1)) {
						$va_all_results = $va_related_objects_by_type[$t_object->get("type_id")];
						$vn_current_obj_index =  array_search($vn_object_id, $va_all_results);
						$vn_current_item_num = $vn_current_obj_index + 1;
...
<?php
						...
						if ($this->getVar('previous_id')) {
							print caNavLink($this->request, "&lt;", '', 'Detail', 'Object', 'Show', array('object_id' => $this->getVar('previous_id')), array('id' => 'previous'));
						}else{
							print "&lt;";
						}
						...
						if ($this->getVar('next_id') > 0) {
							print caNavLink($this->request, "&gt;", '', 'Detail', 'Object', 'Show', array('object_id' => $this->getVar('next_id')), array('id' => 'next'));
						}else{
							print "&gt;";
						}
						print "&nbsp;&nbsp;&nbsp;&nbsp;";
...
<script type="text/javascript">
	function doJump() {
		...
		{search: 'ca_objects.einstellungs_nr:"' + jQuery('#jumpToEinstellungNr').val() + '"'},
